...
cors = require("cors");
const bodyParser = require('body-parser');        //a
const fs = require("fs");

app.use(cors());
app.use(bodyParser.json({ extended: true }));
app.listen(port, () => console.log("Backend server live on " + port));  //b

app.get("/", (req, res) => {
    res.send({ message: "Connected to Backend server!" });  //c
});

app.post("/add/item", addItem) // d

function addItem (request, response) {         //e
    ...
    fs.writeFile("database.json", JSON.stringify(json), function(err, result) {
      ...
      else { console.log('Successfully wrote to file') } // e
    });
    response.send(200)
    }
